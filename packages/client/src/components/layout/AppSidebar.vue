...
import JellyCreditPanel from "./JellyCreditPanel.vue";
...
import { logoutJellyCloudSession } from "@/api/jelly";
...
async function handleLogout() {
  if (jellyManagedMode) {
    await logoutJellyCloudSession().catch(() => undefined);
  }
  localStorage.clear();
...
<template>
  <aside class="sidebar" :class="{ open: appStore.sidebarOpen, collapsed: appStore.sidebarCollapsed }">
    <RouteLinkItem class="sidebar-logo" :to="{ name: 'hermes.chat' }">
      <img :src="logoPath" alt="JellyAI" class="logo-img" />
      <span class="logo-text">JellyAI</span>
    ...
    <nav class="sidebar-nav">
      ...
      <div class="nav-group">
        ...
        <div v-show="!isGroupCollapsed('conversation')" class="nav-group-items">
          ...
          <a v-if="!jellyManagedMode" class="nav-item fun-link" href="https://apikey.fun/register?aff=LIBAPI" target="_blank" rel="noopener noreferrer">
      ...
      <div class="nav-group">
        ...
        <div v-show="!isGroupCollapsed('agent')" class="nav-group-items">
          ...
          <RouteLinkItem v-if="!jellyManagedMode" class="nav-item" :to="{ name: 'hermes.models' }" :active="selectedKey === 'hermes.models'">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
    ...
    <ProfileSelector />
    <ModelSelector v-if="!jellyManagedMode" />
    <JellyCreditPanel v-if="jellyManagedMode" />
    ...
    <div class="sidebar-footer">
      ...
      <div class="version-info">
        <div v-if="!jellyManagedMode" class="version-links">
      ...
      <NButton v-if="!jellyManagedMode && appStore.clientOutdated" type="warning" size="tiny" block class="update-btn" @click="handleReloadClient">
        {{ t('sidebar.reloadClientVersion', { version: appStore.serverVersion }) }}
      </NButton>
      <NButton v-if="!jellyManagedMode && appStore.updateAvailable" type="primary" size="tiny" block class="update-btn" :loading="appStore.updating" @click="handleUpdate">
