...
adminRoutes.get('/api/jelly/admin/summary', async (ctx) => {
  const [accounts, usage, modelGateway] = await Promise.all([
    query<{ active_accounts: string; credits_balance: string }>(
      `SELECT COUNT(*) FILTER (WHERE status = 'active')::text AS active_accounts,
              COALESCE(SUM(credits_balance), 0)::text AS credits_balance
       FROM customer_accounts`,
    ),
    query<{ calls_today: string; credits_today: string }>(
      `SELECT COUNT(*)::text AS calls_today, COALESCE(SUM(credits_cost), 0)::text AS credits_today
...
adminRoutes.get('/api/jelly/admin/accounts', async (ctx) => {
  ctx.body = await query(
    `SELECT id, name, company_name, status, credits_balance, credits_reserved, expires_at, created_at, updated_at
...
adminRoutes.post('/api/jelly/admin/accounts', async (ctx) => {
  try {
    ...
    if (!name || !companyName) throw new Error('NAME_AND_COMPANY_REQUIRED')
    ...
    const account = await transaction(async (client) => {
      const rows = await client.query(
        `INSERT INTO customer_accounts (name, company_name, credits_balance, expires_at)
         VALUES ($1, $2, $3, $4)
         RETURNING id, name, company_name, status, credits_balance, credits_reserved, expires_at, created_at`,
        [name, companyName, creditsBalance, input.expiresAt || null],
      )
      const created = rows.rows[0] as any
      if (creditsBalance > 0) {
        await client.query(
          `INSERT INTO credit_transactions (account_id, type, amount, balance_after, reason)
           VALUES ($1, 'initial_grant', $2, $2, 'Initial account credits')`,
          [created.id, creditsBalance],
        )
      }
      await client.query(
        `INSERT INTO license_keys (account_id, key_prefix, key_hash, key_ciphertext, expires_at)
         VALUES ($1, $2, $3, $4, $5)`,
        [created.id, rawKey.slice(0, 12), hashOpaqueSecret(rawKey), encryptSecret(rawKey), input.expiresAt || null],
      )
      await client.query(
        `INSERT INTO skill_entitlements (account_id, skill_id, enabled)
         VALUES ($1, 'logistics-assistant', TRUE)
         ON CONFLICT (account_id, skill_id) DO UPDATE SET enabled = TRUE, updated_at = NOW()`,
        [created.id],
      )
  ...
  try {
    ...
    ctx.body = await transaction(async (client) => {
      const account = await client.query<{ id: string }>(
        'SELECT id FROM customer_accounts WHERE id = $1 FOR UPDATE',
        [ctx.params.accountId],
      )
      ...
      const existing = await client.query<{
        ...
        `SELECT id, account_id, key_prefix, key_ciphertext, status, expires_at, created_at
         FROM license_keys
         WHERE account_id = $1
         LIMIT 1`,
        [ctx.params.accountId],
        ...
        const rows = await client.query(
          `UPDATE license_keys
           SET key_prefix = $2,
               ...
               last_used_at = NULL
           WHERE id = $1
           RETURNING id, account_id, key_prefix, status, expires_at, created_at`,
          [license.id, rawKey.slice(0, 12), hashOpaqueSecret(rawKey), encryptSecret(rawKey), input.expiresAt || null],
        )
      ...
      const rows = await client.query(
        `INSERT INTO license_keys (account_id, key_prefix, key_hash, key_ciphertext, expires_at)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING id, account_id, key_prefix, status, expires_at, created_at`,
        [ctx.params.accountId, rawKey.slice(0, 12), hashOpaqueSecret(rawKey), encryptSecret(rawKey), input.expiresAt || null],
      )
      return { ...rows.rows[0], licenseKey: rawKey }
    })
  } catch (err) {
...
adminRoutes.get('/api/jelly/admin/accounts/:accountId/licenses', async (ctx) => {
  ctx.body = await query(
    `SELECT id, key_prefix, status, expires_at, created_at, last_used_at
     FROM license_keys WHERE account_id = $1 ORDER BY created_at DESC`,
    [ctx.params.accountId],
  )
})
...
adminRoutes.patch('/api/jelly/admin/accounts/:accountId/licenses/:licenseId', async (ctx) => {
  try {
    ...
    if (status !== null && status !== 'active' && status !== 'disabled') throw new Error('INVALID_LICENSE_STATUS')
    const rows = await query(
      `UPDATE license_keys
    ...
    if (!rows[0]) throw new Error('LICENSE_NOT_FOUND')
    ctx.body = rows[0]
  } catch (err) {
    error(ctx, err)
  }
...
adminRoutes.post('/api/jelly/admin/accounts/:accountId/credits', async (ctx) => {
  try {
    ...
    if (!Number.isInteger(amount) || amount === 0 || !reason) throw new Error('INVALID_CREDIT_ADJUSTMENT')
    ctx.body = await transaction(async (client) => {
      const result = await client.query<{ credits_balance: string }>(
        ...
        [ctx.params.accountId, amount],
      )
      const account = result.rows[0]
      if (!account) throw new Error('INVALID_CREDIT_ADJUSTMENT')
      const rows = await client.query(
        `INSERT INTO credit_transactions (account_id, type, amount, balance_after, reason)
         VALUES ($1, 'admin_adjustment', $2, $3, $4)
         RETURNING id, account_id, type, amount, balance_after, reason, created_at`,
        [ctx.params.accountId, amount, account.credits_balance, reason],
      )
      return rows.rows[0]
    })
  } catch (err) {
...
adminRoutes.get('/api/jelly/admin/accounts/:accountId/transactions', async (ctx) => {
  ctx.body = await query(
    `SELECT id, type, amount, balance_after, reason, request_id, created_at
     FROM credit_transactions WHERE account_id = $1 ORDER BY created_at DESC LIMIT 200`,
    [ctx.params.accountId],
  )
})
...
adminRoutes.get('/api/jelly/admin/accounts/:accountId/usage', async (ctx) => {
  ctx.body = await query(
    `SELECT request_id, model, skill_id, agent_id, prompt_tokens, completion_tokens,
            total_tokens, credits_cost, request_status, error_message, created_at
     FROM usage_logs WHERE account_id = $1 ORDER BY created_at DESC LIMIT 200`,
    [ctx.params.accountId],
  )
})
...
adminRoutes.get('/api/jelly/admin/accounts/:accountId/skills', async (ctx) => {
  ctx.body = await query(
    `SELECT skill_id, enabled, updated_at
     FROM skill_entitlements WHERE account_id = $1 ORDER BY skill_id ASC`,
    [ctx.params.accountId],
  )
})
...
adminRoutes.get('/api/jelly/admin/models', async (ctx) => {
  ctx.body = await query(
    `SELECT model_pricing.id, model_pricing.model, model_pricing.upstream_model,
...
adminRoutes.get('/api/jelly/admin/providers', async (ctx) => {
  ctx.body = await query(
    `SELECT id, name, base_url, enabled, is_default,
...
adminRoutes.put('/api/jelly/admin/providers/:name', async (ctx) => {
  try {
    ...
    if (!name || !baseUrl) throw new Error('PROVIDER_NAME_AND_BASE_URL_REQUIRED')
    ctx.body = await transaction(async (client) => {
      const existing = await client.query<{ api_key_ciphertext: string }>(
        'SELECT api_key_ciphertext FROM model_providers WHERE name = $1',
        [name],
      )
      const apiKeyCiphertext = apiKey ? encryptSecret(apiKey) : existing.rows[0]?.api_key_ciphertext
      if (!apiKeyCiphertext) throw new Error('PROVIDER_API_KEY_REQUIRED')
      if (input.isDefault === true) {
        await client.query('UPDATE model_providers SET is_default = FALSE, updated_at = NOW() WHERE is_default = TRUE')
      }
      const result = await client.query(
        `INSERT INTO model_providers (name, base_url, api_key_ciphertext, enabled, is_default)
         VALUES ($1, $2, $3, $4, $5)
         ...
         RETURNING id, name, base_url, enabled, is_default,
                   (api_key_ciphertext <> '') AS api_key_configured, created_at, updated_at`,
        [name, baseUrl, apiKeyCiphertext, input.enabled !== false, input.isDefault === true],
      )
...
adminRoutes.put('/api/jelly/admin/models/:model', async (ctx) => {
  try {
    ...
    if (!model || !upstreamModel || !providerId) throw new Error('MODEL_UPSTREAM_AND_PROVIDER_REQUIRED')
    ctx.body = await transaction(async (client) => {
      if (input.isDefault === true) {
...
adminRoutes.put('/api/jelly/admin/accounts/:accountId/skills/:skillId', async (ctx) => {
  ...
  ctx.body = (await query(
    `INSERT INTO skill_entitlements (account_id, skill_id, enabled)
     ...
     RETURNING account_id, skill_id, enabled, updated_at`,
    [ctx.params.accountId, ctx.params.skillId, enabled],
  ))[0]
})
