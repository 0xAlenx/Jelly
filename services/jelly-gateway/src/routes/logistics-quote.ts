...
async function log(accountId: string, requestId: string, status: 'success' | 'failed', errorMessage?: string) {
  await query(
    `INSERT INTO usage_logs (
       account_id, request_id, model, skill_id, credits_cost, request_status, error_message
     ) VALUES ($1, $2, 'skill:logistics-quote', $3, 0, $4, $5)`,
    [accountId, requestId, skillId, status, errorMessage?.slice(0, 1000) || null],
  )
}
...
logisticsQuoteRoutes.post('/api/jelly/web/skills/logistics-quote/run', requireAccess('account'), async (ctx) => {
  ...
  try {
    const entitlements = await query<{ enabled: boolean }>(
      'SELECT enabled FROM skill_entitlements WHERE account_id = $1 AND skill_id = $2',
      [accountId, skillId],
    )
    if (entitlements[0]?.enabled !== true) throw new Error('SKILL_NOT_ENTITLED')
    const message = String((ctx.request as any).body?.message ?? '').trim()
    if (!message) throw new Error('LOGISTICS_QUOTE_MESSAGE_REQUIRED')
    if (message.length > 20_000) throw new Error('LOGISTICS_QUOTE_MESSAGE_TOO_LONG')
