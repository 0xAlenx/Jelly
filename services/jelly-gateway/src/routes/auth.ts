...
async function resolveLicense(rawKey: string): Promise<LicenseRow> {
  const rows = await query<LicenseRow>(
    `SELECT license_keys.id, license_keys.account_id
     FROM license_keys
     ...
     WHERE license_keys.key_hash = $1
       ...
       AND (customer_accounts.expires_at IS NULL OR customer_accounts.expires_at > NOW())`,
    [hashOpaqueSecret(rawKey)],
  )
...
authRoutes.post('/api/jelly/auth/license-login', async (ctx) => {
  try {
    const licenseKey = String((ctx.request as any).body?.licenseKey ?? '').trim()
...
authRoutes.post('/api/jelly/auth/device-activate', async (ctx) => {
  try {
    ...
    const license = await resolveLicense(licenseKey)
    await query(
      `INSERT INTO activated_devices (account_id, device_id, device_name)
       VALUES ($1, $2, $3)
       ON CONFLICT (account_id, device_id)
       DO UPDATE SET device_name = EXCLUDED.device_name, status = 'active', last_seen_at = NOW()`,
      [license.account_id, deviceId, deviceName || null],
    )
...
authRoutes.post('/api/jelly/auth/refresh', async (ctx) => {
  try {
    const refreshToken = String((ctx.request as any).body?.refreshToken ?? '').trim()
...
authRoutes.get('/api/jelly/me', requireAccess('account'), async (ctx) => {
  try {
    ...
    const skills = await query<{ skill_id: string }>(
      'SELECT skill_id FROM skill_entitlements WHERE account_id = $1 AND enabled = TRUE ORDER BY skill_id',
      [account.id],
    )
    const usage = await query<{ credits_today: string }>(
      `SELECT COALESCE(SUM(credits_cost), 0)::text AS credits_today
       FROM usage_logs WHERE account_id = $1 AND created_at >= CURRENT_DATE`,
      [account.id],
    )
    ctx.body = {
      id: account.id,
