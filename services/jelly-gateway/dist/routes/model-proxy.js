...
import { createRequestMetadata, releaseCredits, reserveCredits, settleCredits, } from '../billing.js';
...
function usageFromObject(payload) {
...
async function providerRoute(pricing) {
    const rows = await query(`SELECT base_url, api_key_ciphertext
    ...
    if (pricing.provider_id)
        throw new UpstreamRouteError('MODEL_PROVIDER_NOT_AVAILABLE');
    if (!config.modelBaseUrl || !config.modelApiKey)
        throw new Error('MODEL_GATEWAY_NOT_CONFIGURED');
...
async function fetchUpstream(provider, path, metadata, payload) {
    try {
...
async function forwardJson(ctx, path, accountId, metadata, pricing, provider, payload) {
...
async function forwardStream(ctx, path, accountId, metadata, pricing, provider, payload) {
    ...
    if (!response.ok)
        throw await upstreamError(response);
    if (!response.body)
        throw new UpstreamRouteError('UPSTREAM_STREAM_BODY_MISSING');
    let latestUsage = { promptTokens: 0, completionTokens: 0 };
    let buffer = '';
    let finalized = false;
    async function releaseOnce(reason) {
        if (finalized)
            return;
        finalized = true;
        try {
...
async function proxy(ctx, path, kind = 'chat') {
    ...
    try {
        ...
        if (kind === 'skill' && !metadata.skillId)
        ...
        for (const candidate of reserved.pricingCandidates) {
            ...
            try {
                ...
                if (payload.stream)
                    await forwardStream(ctx, path, accountId, metadata, pricing, provider, payload);
                else
                    await forwardJson(ctx, path, accountId, metadata, pricing, provider, payload);
                return;
            }
            catch (err) {
                if (!(err instanceof UpstreamRouteError))
                    throw err;
