...
function pythonBin() {
    if (config.logisticsQuotePythonBin)
        return config.logisticsQuotePythonBin;
...
async function executeQuote(accountId, message) {
    ...
    if (!existsSync(script))
        throw new Error('LOGISTICS_QUOTE_WORKER_NOT_CONFIGURED');
    try {
        ...
        if (!reply)
            throw new Error('LOGISTICS_QUOTE_EMPTY_RESPONSE');
        return reply;
    }
    catch (err) {
        const code = err instanceof Error ? err.message : 'LOGISTICS_QUOTE_FAILED';
        if (code.startsWith('LOGISTICS_QUOTE_'))
            throw err;
        throw new Error('LOGISTICS_QUOTE_FAILED');
    }
}
export async function runLogisticsQuote(accountId, message) {
